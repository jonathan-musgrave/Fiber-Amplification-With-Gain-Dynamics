function sys = plot_FiberPropResults(sys,signal,pumps,fiber)
        ...
        subplot(5,2,2);hold on;
            ...
            xlabel('\lambda (nm)'); ylim([0,1.2]); ylabel('Spectrum')
            [~,lamINX] = max((2*pi*c./(fftshift(sys.w)+2*pi*c./sys.lambda0)).*fftshift(signal.Iw(end,:)))
            lambda0 = 2*pi*c./(fftshift(sys.w)+2*pi*c./sys.lambda0)
            lambda0 = lambda0(lamINX)
            xlim([-100,100]+lambda0./nm)
